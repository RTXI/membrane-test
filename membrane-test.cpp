...
void MembraneTest::execute(void) {
  ...
  switch (mem_mode) {
    ...
    // collects mem_steps * hold_count data points and stores them for later
    // analysis
    case DETAILED:
      if (hold_index < hold_count / 2) {
      ...
      if (!(++hold_index %= hold_count)) {
        ...
        // number of steps saved (each element has mem_steps points added to it)
        // == mem_steps
...
void MembraneTest::customizeGUI(void) {
  ...
  QObject::connect(zap_button, SIGNAL(clicked(void)), this,
                   SLOT(toggleZap(void)));
  QObject::connect(membuttonsgroup, SIGNAL(buttonClicked(int)), this,
                   SLOT(selectMode(int)));
  QObject::connect(voltbuttonsgroup, SIGNAL(buttonClicked(int)), this,
                   SLOT(selectVoltage(int)));
  ...
  QObject::connect(label_timer, SIGNAL(timeout(void)), this,
                   SLOT(computeMembraneProperties(void)));
  ...
  switch (mem_mode) {
    ...
    case DETAILED:
      ...
      double tau1;
      {
        ...
        gsl_linalg_SV_solve(&a.matrix, &b.matrix, &c.vector, &e.vector,
                            &d.vector);
        tau1 = fabs(1.0 / x[1]);
      }
...
void MembraneTest::updateUIResistance(void) {
  ...
  // figure out which exponent is needed (k, M G, or T) for displaying the Ohm
  // units
  if ((R != INFINITY) && (R != NAN)) {
    while (R >= 1e3) {
      R *= 1e-3;
